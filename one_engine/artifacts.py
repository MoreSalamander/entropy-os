...
_CONVENTIONS = {
    "research": ("ONE_ENGINE_ARTIFACTS_RESEARCH", "storage_data/reports", "report"),
    "software": ("ONE_ENGINE_ARTIFACTS_SOFTWARE", "storage_data/projects", "project"),
    "web": ("ONE_ENGINE_ARTIFACTS_WEB", "storage_data/sites", "site"),
    "university": ("ONE_ENGINE_ARTIFACTS_UNIVERSITY", "storage_data/lessons", "lesson"),
}
...
def _root_for(engine: str, repos: dict[str, str]) -> Path | None:
    ...
    return Path(repo) / subpath if repo else None
...
def _by_convention(engine: str, produced: dict[str, Any],
                   repos: dict[str, str]) -> tuple[str, str] | None:
    """(path, kind) for a stage that recorded ids but not paths."""
    root = _root_for(engine, repos)
    entry = _CONVENTIONS.get(engine)
...
def resolve(stages: list[dict[str, Any]], repos: dict[str, str],
    ...
    for stage in stages:
        ...
        guess = _by_convention(engine, produced, repos)
        if guess is None:
            continue
        path, kind = guess
...
def repos_from_config(cfg: Any) -> dict[str, str]:
    """Engine key -> repo root, as the running topology describes it."""
    return {name: member.repo for name, member in cfg.engines.items()}
