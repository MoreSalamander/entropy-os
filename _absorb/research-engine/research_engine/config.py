...
Single source of truth for runtime settings: config.yaml overlaid with
environment variables (env wins, so keys never need to live on disk).
...
def load_config(path: str | Path | None = None) -> Config:
    """Load config.yaml (project root by default), overlay env, return typed Config."""
