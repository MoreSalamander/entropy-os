...
def list_concepts(session: Session) -> list[models.Concept]:
    return list(session.scalars(select(models.Concept)).all())


def get_concept(session: Session, item_id: int) -> models.Concept | None:
    return session.get(models.Concept, item_id)


def create_concept(session: Session, data: dict) -> models.Concept:
    obj = models.Concept(**data)
    session.add(obj)
    session.commit()
    session.refresh(obj)
    return obj
...
def delete_concept(session: Session, item_id: int) -> bool:
    obj = session.get(models.Concept, item_id)
    if obj is None:
        return False
    session.delete(obj)
    session.commit()
