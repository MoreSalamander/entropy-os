...
# Two roots after the split: the veritas checkout (where the suite runs and the
# engine docs' proof points live) and this repo's UI (where the viz data lands).
ROOT = Path(os.environ.get("VERITAS_DIR", Path(__file__).resolve().parent.parent.parent / "veritas"))
UI_ROOT = Path(__file__).resolve().parent.parent / "entropy_os"
...
def write_nerve_data(categories: list[dict]) -> None:
    out = UI_ROOT / "static" / "shared" / "nerve-viz-data.js"
    out.write_text(
