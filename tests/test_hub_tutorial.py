...
def _client(tmp_path, provider, monkeypatch, *, build_ok: bool = True):
    if build_ok:
        ...
        monkeypatch.setattr("entropy_os.app.build_tutorial_image", _boom)
    monkeypatch.setattr(
        "entropy_os.app.publish_tutorial",
...
def test_a_failed_mirror_does_not_erase_the_native_product(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("entropy_os.app.publish_tutorial", _boom)
...
def test_dispense_runs_a_copy_of_the_products_own_container(tmp_path, monkeypatch):
    ...
    returned = {"id": None}
    monkeypatch.setattr("entropy_os.app.return_copy", lambda cid: returned.__setitem__("id", cid))
