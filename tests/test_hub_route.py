...
def _client(router_json: str | None) -> TestClient:
    ...
    return TestClient(create_app(data_dir=Path(tempfile.mkdtemp()),
                                 provider=ScriptedProvider(by_role)))
...
def test_keyword_fallback_when_model_unavailable():
    ...
    assert c.post("/api/route",
                  json={"request": "a short animated film"}).json()["org"] == "production"
    assert c.post("/api/route",
                  json={"request": "a landing page for a cafe"}).json()["org"] == "web"
...
def test_invalid_model_org_falls_back_to_keywords():
    ...
    assert c.post("/api/route",
                  json={"request": "a function that sorts a list"}).json()["org"] == "software"
