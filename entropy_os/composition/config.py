"""System topology loader for the composed engine.

Reads `config/composition.yaml`. Kept as plain dataclasses (not pydantic)
...
from ..paths import COMPOSITION_CONFIG, COMPOSITION_STORAGE, EVENTS_LOG


@dataclass
class MemberConfig:
    """How to reach one member. An address and nothing else — a member is
    defined by the contract it answers, not by where its code sits."""
    name: str
    port: int = 0
    url: str = ""
...
def load_config(path: Path | None = None) -> SystemConfig:
    ...
    """
    raw = yaml.safe_load((path or COMPOSITION_CONFIG).read_text())
    cfg = SystemConfig()
    for name, e in (raw.get("engines") or {}).items():
        cfg.engines[name] = MemberConfig(
            name=name, port=int(e.get("port", 0)),
    ...
    if ev.get("log_path"):
        p = Path(ev["log_path"])
        # Relative paths resolve against the storage root, not the checkout:
        # the log is state, and state moves with ENTROPY_STORAGE.
        cfg.events_log_path = p if p.is_absolute() else COMPOSITION_STORAGE / p
